...
export function renderSidebar(manager) {
  ...
  document
    ...
    .addEventListener("click", () => showProjectForm(manager));
...
export function addProjectToDom(manager, project) {
  ...
  const div = dom.newElement("div", null, `project${index}`, [
    "sidebar-project",
  ]);
  ...
  delBtn.addEventListener("click", () => {
    ...
    manager.deleteProject(
      manager.projects.findIndex((p) => p.id === project.id),
    );
...
export function showTasks(manager, project) {
  ...
  const taskHeader = dom.newElement("div", null, null, [
    "task-container-header",
  ]); // Init the task header
...
export function showNoTasks(manager, project) {
  ...
  taskHeader.appendChild(
    dom.newElement("h2", "No tasks...", null, ["task-header"]),
  );
...
export function showNoProjects() {
  ...
  taskHeader.appendChild(
    dom.newElement("p", "Add a project to get started", null, ["task-header"]),
  );
  taskContainer.appendChild(taskHeader);
}
...
export function showTaskForm(manager, project) {
  ...
  // Create the validation message
  const titleValidationMsg = dom.newElement("p", "Max Characters: 25", null, [
    "validation-msg",
  ]);
  ...
  const description = dom.newElement("textarea", null, null, [
    "form-description",
  ]);
  form.appendChild(description);
  // Create the validation message
  const descriptionValidationMsg = dom.newElement("p", "", null, [
    "validation-msg",
  ]);
